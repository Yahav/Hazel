...
module.exports = class Cache {
  ...
  async refreshCache() {
    ...
    if (!Array.isArray(data) || data.length === 0) {
      console.log('refreshCache !Array.isArray(data) || data.length === 0', response)
      return
    }
    ...
    const release = data.find(item => {
      console.log('refreshCache data.find item.draft', !item.draft)
      console.log('refreshCache pre', pre, Boolean(pre))
      console.log('refreshCache data.find item.prerelease', item.prerelease, Boolean(item.prerelease))
      const isPre = Boolean(pre) === Boolean(item.prerelease)
      console.log('refreshCache !item.draft && isPre', (!item.draft && isPre))
      return !item.draft && isPre
    })

    if (!release || !release.assets || !Array.isArray(release.assets)) {
      console.log('refreshCache !release || !release.assets || !Array.isArray(release.assets)', release)
      return
    }
